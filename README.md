...
- CFB는 다른 비디오 조각에 대해 나타나는 비디오 콘텐츠의 의미론적 "스니펫"을 제공한다. 
초기 파일럿 연구는 사용자가 맥락을 유지할 수 있도록 하는 것이 중요하다는 것을 밝혔다. 

따라서 동영상을 클릭할 때 새 페이지를 여는 대신 동영상 플레이어를 표시하는 데 팝업이 사용되었습니다(그림 2의 오른쪽 하위 그림 참조). 

- CFB의 계단식 메뉴(그림 1의 녹색 사각형으로 강조 표시)는 특정 타임스탬프에 있는 비디오의 키워드를 표시하므로 
사용자는 콘텐츠를 클릭하지 않고도 비디오에 포함된 주제를 탐색할 수 있습니다. 
...
그러면 비디오 플레이어 창이 열리고 이 특정 조각에서 비디오가 시작됩니다. 

- 또한 모든 참가자는 비디오 타임라인을 따라 커서를 이동하여 비디오를 탐색할 수 있습니다.
...
- 결론 : 그렇게 함으로써, 우리의 접근 방식은 도메인에 구애받지 않고, 가장 크고 끊임없이 진화하는 지식 기반인 위키피디아를 활용하여 값비싼 전문가 레이블링의 필요성을 피하는 동시에, 여러 언어로 작업하고 설명과 정밀 조사를 가능하게 하는 인간적으로 해석 가능한 주석을 가능하게 한다[3].

- [참고] 엔티티 연결 태스크에 대한 성능을 포함하여 위키화에 대한 자세한 내용은 [4]를 참조하십시오.
