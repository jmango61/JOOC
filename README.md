...
- 그러나 **비디오 콘텐츠를 미리 볼 때,** 일반적으로 제목 및 설명과 같은 **측면 정보**는 의미론적으로 **관련된 섬네일을 만들기 위해 사용**됨[35, 40].

- 애니메이션 섬네일 그리드[20] 및 수많은 진행 표시줄 기반 “skimming” 접근 방식과 같은 변형은 YouTube 또는 Netflix와 유사한 미리 보기를 생성함[11, 26].

- **'skimming'**은 **교육 비디오를 미리 볼 때 젊은 성인 자기 주도 학습자 사이에서 일반적인 전략**인 것으로 밝혀짐[25]. 

② 비디오를 미리 보는 것이 주로 섬네일로 연상되는 동안, **연구는 텍스트 메타데이터와 미리 보기 모두를 사용하는 것이 더 효율적인 관련성 판단으로 이어진다**는 것을 보여줌[16].
-> 이러한 발견을 기반으로 **메타데이터, 스크립트 기반 개념 주석 및 미리 보기를 사용하여 효율적인 미리 보기 및 탐색을 개선**

- 여러 기술은 IR을 개선하기 위해 비디오의 주석을 사용하는 것을 탐구.

- 이들 중 일부는 크라우드 소싱을 사용하여 시간 지정 메타데이터[30] 및 비디오 조각 개념 주석[28, 31]과 같은 비디오 정보를 수집하는데, 이는 상당한 인력 없이는 확장 불가능.

- 이를 위해 비디오 검색 작업을 위해 신경 [27] 및 의미 기반 [39] 자동 비디오 표현 도구가 제안됨. 

- **한계** : 그러나 이러한 기법 중 어느 것도 **관련성 판단을 위한 비디오 미리보기의 유용성 측면에서 연구되지 않음.** 

③ YouTube Chapters(비디오 세그먼트에 대한 텍스트 제목)
- 이는 최근 자동화된 비디오 조각 주석을 가지고 있음.
- 이 도구는 비디오 내에서 비선형 탐색을 허용하나, 우리의 접근 방식은 **비디오 검색 결과를 탐색할 때 스마트 미리 보기를 허용**<br/>

= 즉, 비디오를 선택하기 전에 사용 가능 <br/> 
=  YouTube Chapters는 비디오 프레임에서 텍스트를 식별하기 위해 광학 문자 인식을 사용하여 프레젠테이션과 같은 (슬라이드 쇼) 비디오에 대한 접근을 제한 <br/> 

- 문제점 <br/> 
: 도구(YouTube Chapters)의 설계 및 실험 결과는 공개되지 않아 성공 평가를 제한<br/> <br/> 

- 본 연구 아이디어의 장점 ( 도구(YouTube Chapters)와 비교했을 때 ) <br/> 
   - **위키백과 주제**[4]를 사용하여 **자동 개념 주석을 활용** -> **사용자**가 **위키백과 정의[15]를 통해** 검색 중에 **낯선 주제에 대한 지식을 확장**할 수 있음 <br/> 
   - **스크립트 기반**이므로 **모든 유형의 정보 비디오로 작업 가능** <br/><br/>  

- 사용자 연구는 또한 제안된 **지능형 비디오 미리보기 도구**에 대해 **매우 필요한 사용적합성 평가를 제공**하여 **정보 비디오 검색에서 탐색 도구의 잠재력을 보여줌.**
...
CFB는 누구나 온라인 공개 교육 자료를 볼 수 있도록 설계된 교육 플랫폼 2에 배치되었습니다.

 의도된 기능 중 하나는 학습자와 교육자가 자체 커리큘럼을 구성할 수 있도록 하는 것이다. 예를 들어, 공개 교육 자료를 검색하고 내용의 잠재적 관련성을 판단하는 것이다. 

학습자와 교육자 모두를 위해 설계되었지만, 여기서는 후자 그룹에 대한 유용성과 유용성을 평가한다. 

플랫폼은 검색 도구, CFB 및 계단식 메뉴와 함께 그림 1에서 볼 수 있습니다. 이 스크린샷은 사용자가 플랫폼에 가입하여 '기계 학습'이라는 주제를 검색할 때 무엇을 볼 수 있는지 보여준다.

시스템이 반환하는 결과 비디오는 축소판 보기로 표시됩니다. 

CFB는 각 비디오에 통합되어 있어 시간에 따라 다룬 (위키피디아) 주제를 비디오로 시각화하고 쿼리와 관련하여 각 비디오의 가장 관련성이 높은 부분을 강조 표시할 수 있다.

CFB에는 위키백과 개념에 대한 정의도 포함되어 있어 사용자가 이 용어를 잘 모를 경우 접근할 수 있다. 

직관적인 사용을 위해, 우리의 디자인은 익숙한 UX 패턴과 기술을 활용하는데, 주로 팝업, 컬러 코딩 위젯 및 계단식 메뉴를 사용한다. 이 플랫폼에는 CFB와 함께 사용하도록 설계된 다른 통합 기능/도구도 포함되어 있습니다. 

이러한 것들은 [6, 29]에 설명되며 학습 경험을 지원하는 것을 목표로 한다(예: 학습 경로의 최적화, 비디오의 주석, 추천 시스템 [8, 9 등).
...
- 각 작업에 대해 비디오 표시 
- 오른쪽에는 비디오를 클릭하면 열리는 비디오 플레이어 화면(팝업) 표시


### 3.2 Content Flow Bar (CFB)

CFB는 다른 비디오 조각에 대해 나타나는 비디오 콘텐츠의 의미론적 "스니펫"을 제공한다. 초기 파일럿 연구는 사용자가 맥락을 유지할 수 있도록 하는 것이 중요하다는 것을 밝혔다. 

따라서 동영상을 클릭할 때 새 페이지를 여는 대신 동영상 플레이어를 표시하는 데 팝업이 사용되었습니다(그림 2의 오른쪽 하위 그림 참조). 

CFB의 계단식 메뉴(그림 1의 녹색 사각형으로 강조 표시)는 특정 타임스탬프에 있는 비디오의 키워드를 표시하므로 사용자는 콘텐츠를 클릭하지 않고도 비디오에 포함된 주제를 탐색할 수 있습니다. 

CFB의 일부로서 키워드의 정의도 팝업 형태로 제공된다. 이러한 키워드와 정의는 나중에 설명하는 위키화 프로세스에서 자동으로 추출되었습니다.

CFB는 쿼리에 대한 각 조각의 예측 관련성을 나타내는 다른 색상 강도(노란색, 그림 1 참조)를 사용하여 추가로 지원됩니다. 이러한 친숙한 시각적 인터페이스 요소를 결합하는 것은 사용자가 미리 보기 행렬을 빠르게 훑어보고 가장 적절한 것을 선택할 수 있도록 하기 위한 것이었다. 

전반적으로 CFB에 사용되는 큐잉의 유형은 다음을 가능하게 하기 위한 것이다. 
사용자는 비디오 강의에서 다루는 내용을 한 눈에 확인하고 특정 지점에서 멈추고 더 많은 내용을 발견할 수 있습니다. 

참가자는 메인 창에서 비디오 조각을 더 자세히 보고 싶을 때 해당 조각으로 바로 이동할 수 있습니다. 

그러면 비디오 플레이어 창이 열리고 이 특정 조각에서 비디오가 시작됩니다. 또한 모든 참가자는 비디오 타임라인을 따라 커서를 이동하여 비디오를 탐색할 수 있습니다.

### 3.3 Wikification to Extract Keywords

우리의 도구는 TransLectures 프로젝트3의 전사 및 번역 알고리즘을 사용하여 비디오의 텍스트 표현을 얻는다. 

위키화 프로세스에서, 자원의 영어 텍스트 표현(필요할 때 번역)은 이전 작업이 그러한 단편화가 적절하다고 판단하기 때문에, 긴 문서를 약 5000자(비디오에서 약 5분)의 단편으로 분할하는 데 사용된다[9]. 

그런 다음 각 조각은 자연어 처리 엔티티 연결 방법을 사용하여 관련성이 있고 두드러진 위키백과 개념으로 텍스트를 레이블링하는 openWikifier 서비스4를 사용하여 자동으로 주석을 달 수 있다[4 

그렇게 함으로써, 우리의 접근 방식은 도메인에 구애받지 않고, 가장 크고 끊임없이 진화하는 지식 기반인 위키피디아를 활용하여 값비싼 전문가 레이블링의 필요성을 피하는 동시에, 여러 언어로 작업하고 설명과 정밀 조사를 가능하게 하는 인간적으로 해석 가능한 주석을 가능하게 한다[3].

 엔티티 연결 태스크에 대한 성능을 포함하여 위키화에 대한 자세한 내용은 [4]를 참조하십시오.
 
---
## 4. User Evaluation

도구를 설계하는 동안 우리는 CFB가 있는 것과 없는 두 가지 버전의 정보 검색 작업을 받은 8명의 참가자를 대상으로 예비 사용자 연구(실제 연구 이전)를 수행했다. 

정성적 피드백은 모든 참가자들이 도구가 비디오에서 정보를 찾는 데 유용하고 직관적이며 유용하다는 것을 발견했음을 나타냅니다.

 특히 콘텐츠를 알 수 없는 긴 영상으로 장점이 두드러졌다. 

한 참가자는 "영상을 모르면, 내용이 어디에 있는지, 하나의 생각의 시작과 끝이 어디에 있는지 아는 것은 매우 어렵다. [...] [CFB] 없이 나는 그것이 내가 관심 있는 지점으로 가기까지 오랜 시간을 지켜봐야 했다."라고 말했다.

CFB를 통해 참가자는 "실제로 관심 있는 부분으로 건너뛸 수 있기 때문에" 작업이 더 쉽고 더 즐겁다는 것을 알게 되었습니다.


### 4.1 Controlled User Study

정보 검색 및 수집을 위한 CFB의 효과를 평가하기 위해 26명의 참가자를 대상으로 사용자 연구를 수행하였다.

### 4.2 Goal of Study and Research Questions

주요 사용자 연구는 CFB(제어 조건)가 있는 비디오 플레이어의 사용자 성능을 비교하기 위해 설계되었다.

좀 더 구체적으로 말하면, 제어 비디오 플레이어는 유튜브를 기반으로 호버링할 때 변화하는 섬네일을 보여주고 시청된 부분을 강조 표시했다(그림 3 참조). 

향상된 비디오 플레이어는 CFB를 가지고 있었는데, 여기에는 각 단편에서 다루는 주제 목록과 그 정의(그림 2 참조)가 포함되어 있었다. 

이 비교 접근법은 비디오 플레이어 상호 작용을 평가할 때 사용되는 일반적인 방법입니다 [22, 37].

우리는 기계 학습과 기후 변화와 관련된 두 가지 과제를 설계했다. 

기계 학습은 학문적으로 인기 있는 주제이기 때문에 선택되었다. 기후 변화는 보편적인 관심사 때문에 선택되었다. 이 두 가지 모두 주제이며 CFB가 통합된 플랫폼에서 사용할 수 있는 많은 비디오가 있습니다.

둘 다 참가자들이 각 도메인을 가르치는 데 유용한 관련 동영상을 찾도록 지시받은 정보 찾기 작업이었다. 

우리는 정보 작업이 더 어렵고 시간이 많이 걸리는 경향이 있기 때문에 탐색 작업보다 정보 작업을 선택했다. 과제가 공개되었고 참가자들은 각 과제를 완료하는 데 필요한 만큼의 시간을 가질 수 있었다. 

이는 또한 참가자들이 특정 시간에 작업을 완료하라는 지시로 준비되지 않고 자신의 시간에 비디오를 탐색하고 도구를 찾을 수 있도록 개방형 방식으로 작업을 완료하는 시간을 측정하는 데 관심이 있었기 때문이다.

이 연구의 목적은 CFB가 정보 찾기를 지원하는지 여부와 방법을 파악하여 콘텐츠 탐색을 촉진하고 비디오 콘텐츠의 효율적인 브라우징을 지원하는 데 있었다. 

본 연구를 위한 두 가지 주요 연구 질문은 다음과 같다.
• 내비게이션과 브라우징은 두 인터페이스에서 어떻게 비교됩니까?
• 정보 추구와 탐색적 행동의 측면에서 어떤 차이가 있습니까? 

4.2.1 사용자 조치. 
연구 질문에 답하기 위해, 우리는 다음과 같은 다섯 가지 조치에 초점을 맞추어 성능과 사용자 경험의 특정 측면에 특히 주의를 기울인다. 

(i) 동영상 열기, 즉 참가자가 동영상을 클릭하기로 결정했을 때 비디오 플레이어 화면이 열림 
(ii) 비디오 플레이어 화면에서 비디오 재생/재생 
(iii) 비디오 탐색(결과 화면 또는 비디오 플레이어 화면에서) CFB의 키워드 또는 축소판 그림 프레임과 같은 비디오의 내용을 탐색하는 것을 말한다. 

(iv) 관련 비디오 세그먼트의 선택, 즉, 사용자가 비디오의 일부가 당면한 작업과 관련이 있다고 판단하고 작업 공간에 관련 세그먼트를 저장하는 경우 및 v) 비디오 내에서 찾는 정보 [14], 즉, 검색 작업은 사용자가 비디오 내에서 탐색하는 경우입니다(재생할 비디오의 여러 시간 위치에서 이동/이동).

비디오의 일부를 열고 재생하고 선택하는 동작은 두 조건(CFB ON/OFF)에서 동일합니다. 

그러나 탐험은 다르다. 

두 경우 모두 비디오 아래의 시계열 표시줄(결과 및 비디오 플레이어 화면에 모두 있음)과 상호 작용하여 콘텐츠를 탐색합니다. 

CFB가 꺼져 있을 때, 탐험 경험은 유튜브의 주류 비디오 플레이어와 유사하게 유지됩니다. 

결과 화면에서 시계열 막대 위를 맴돌면 마우스 커서에 해당하는 비디오의 시간 위치가 표시됩니다. 

또한, 시간 위치에 대응하는 프레임들을 미리 보는 비디오 섬네일이 디스플레이된다. 

비디오 화면에서는 축소 이미지 표시가 비활성화되어 있는 동안 시간 위치가 유지됩니다. 비디오에서 본 부분은 빨간색으로 표시되어 있다. 

그림 3은 CFB가 꺼졌을 때의 비디오 플레이어를 보여줍니다. 

CFB가 켜져 있는 경우 시간이 위키백과 키워드와 정의가 있는 CFB 팝업으로 대체된다는 점이 유일한 차이점이다. 

CFB 꺼짐 상태와 유사하게 축소판 미리 보기는 결과 화면에서 활성화되고 비디오 화면에서 비활성화됩니다.

![그림3](https://user-images.githubusercontent.com/100738522/199811141-773b05cd-76af-4021-9030-954860446695.png)
**[그림3] CFB가 꺼진 비디오 플레이어**
- 타임라인의 커서 : 시간 위치
- 빨간색 대시와 점 : 감시된 부품
