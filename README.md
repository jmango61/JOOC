...
[그림2 설명]
그림 2: 연구의 스크린샷, 중요한 구성 요소 강조 왼쪽 하위 그림은 결과 화면을 보여 줍니다.
각 작업에 대해 비디오가 표시됩니다. 오른쪽에는 비디오를 클릭하면 열리는 비디오 플레이어 화면(팝업)이 표시됩니다.
...
[그림3 설명]
그림 3: CFB가 꺼진 비디오 플레이어 
타임라인의 커서는 시간 위치를 나타냅니다. 빨간색 대시와 점은 감시된 부품을 나타냅니다.
...
[그림4 설명]
그림 4: 작업 순서와 주제 간의 상호 작용 행동의 차이를 나타내는 상호 작용 메트릭의 하위 집합의 상자 그림
 
이러한 그림(특히 하위 그림 a와 b)은 사용자가 첫 번째 작업에 훨씬 긴 시간을 소비하는 경우 학습 효과가 발생함을 시사한다. 

하위 그림 c)는 사용자가 유사한 시간을 보내고 두 주제에 대해 동일한 수의 세그먼트를 선택하도록 권장되었지만 두 가지 다른 작업에 대해 선택된 세그먼트의 수는 다르다는 것을 보여준다.
...
[그림5 설명]
그림 5: 선택한 질문에 대한 응답
사용자 설문지 6등급 Likert 척도가 사용됩니다. 다른 색조의 파란색이 동의 응답에 사용되는 반면 다른 색조의 보라색은 동의하지 않는 응답에 사용됩니다.
주제 분석. 
