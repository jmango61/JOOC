...
① 동영상 열기, 즉 참가자가 동영상을 클릭하기로 결정했을 때 비디오 플레이어 화면이 열림 <br/>
② 비디오 플레이어 화면에서 비디오 재생/재생 <br/>
③ 비디오 탐색(결과 화면 또는 비디오 플레이어 화면에서) CFB의 키워드 또는 축소판 그림 프레임과 같은 비디오의 내용을 탐색하는 것<br/>

④ 관련 비디오 세그먼트의 선택, 즉, 사용자가 비디오의 일부가 당면한 작업과 관련이 있다고 판단하고 작업 공간에 관련 세그먼트를 저장하는 경우 및 <br/>
...
- 위 연구는 **반복 측정 설계** 사용
   - 각 참가자는 CFB와 통제 조건을 사용하여 두 가지 작업을 모두 수행함. (참가자들이 두 가지 조건과 과제를 제시한 순서는 무작위로 결정되었음.)
   - CFB 조건은 연습 과제에서 입증되었음.


- 연구 수행 플랫폼 : ZOOM 웹 회의 플랫폼
...
5) 그 후 그들은 CFB에 대한 설문지를 작성하도록 요청받음.


- Q. 사용자에게 세 가지 작업(실습, 작업 1 및 작업 2)에 **세 가지 다른 주제 (뇌, 기후 변화 및 머신 러닝)가 사용되는 이유?**
   - A. 사용자가 **여러 작업에 걸쳐 지식 영역에 익숙**해진다면, 포착되고 분석되는 **상호 작용 행동에 영향**을 미칠 수 있으므로 **이러한 학습 효과를 피하기 위함임.**
...
- 각 과제(기계 학습 및 기후 변화)에 대해, 참가자들은 연구를 위해 신중하게 선택된 18개의 비디오와 함께 결과 화면에 제시됨.

- 그림2(왼쪽)에 표시된 비디오 결과 집합은 무작위로 순위가 매겨졌음. (각 참가자가 동일한 순서로 비디오를 볼 수 있도록 모든 참가자들 사이에서 일정하게 유지)
   - 즉, 비디오는 작업 주제와의 관련성과 무관하게 정렬되었음. 
   - 그러나 모든 비디오는 작업 해결과 관련이 있도록 선택됨. 

- 베이스라인 비디오 플레이어는 YouTube를 기반으로, 맴돌 때 변화하는 섬네일을 보여주며, 시청한 부분을 빨간색 점으로 강조 표시. 

- 향상된 비디오 플레이어는 CFB를 가지고 있었는데, 여기에는 각 단편에서 다루는 주제 목록과 그 정의들이 포함되어 있음.

- 그림 2에 있는 사용자 연구 인터페이스에서 CFB의 관련성 기반 단편 강조 기능이 비활성화 됨.
   - why? 사용자 연구가 정적 결과 집합으로 제한됨에 따라 (사용자 쿼리 기반 동적 결과 집합과는 달리)

 - 참가자가 비디오를 클릭 -> 비디오 플레이어가 열림 & 참가자가 비디오 플레이어 화면으로 이동. (그림 2 참조)
 - (참가자들에게) 제공된 기능 : 비디오 클립을 선택하고 작업 공간에 저장할 수 있는 기능 

- 참가자가 비디오 클립을 선택 -> 선택한 세그먼트가 CFB에서 파란색으로 강조 표시 & 비디오 제목이 참가자의 작업 영역에 표시.
   - 참가자는 작업영역에서 선택 항목을 삭제 가능.
...
- 선택된 비디오 세트가 주어진 과제를 달성할 수 있는지 확인.
...
인구통계학 및 지식과 관련된 설문지의 이 부분은 참가자가 도구와 상호 작용하기 전에 수행되었다. 
CFB와 관련된 질문이 포함된 설문지의 두 번째 부분은 참가자들이 연구에 참여한 후에 질문되었다. 
